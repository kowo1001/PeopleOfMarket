...
<<<<<<< HEAD
	public static Orders createOrder(int orderId, int orderNumber,int productNumber, int amount, int totalPrice, String pickupDate, String pickupTime, char ispickup, EntityManager em) throws SQLException {
		EntityTransaction tx = em.getTransaction();
		Orders o = null;
		
		tx.begin();
		o = Orders.builder().orderId(orderId).orderNumber(orderNumber).productNumber(productNumber).amount(amount).totalPrice(totalPrice).pickupDate(pickupDate).pickupTime(pickupTime).ispickup(ispickup).build();
		em.persist(o);
		tx.commit();
		return o;
	}
	...
	public static Orders findOrdersByOrderId(int orderId, EntityManager em)  throws SQLException, NoResultException  {
		Orders o = (Orders) em.createNativeQuery("select * from orders where oid=?", Orders.class)
				.setParameter(1,orderId).getSingleResult();
		return o;
	}
	
	public static Orders findOrdersByOrderNumber(int orderNumber, EntityManager em)  throws SQLException, NoResultException  {
		Orders o = (Orders) em.createNativeQuery("select * from orders where ono=?", Orders.class)
				.setParameter(1,orderNumber).getSingleResult();
		return o;
	}
	
	public static boolean updateOrders(int orderId,String pickupDate, String pickupTime, EntityManager em) throws SQLException, NoResultException {
		EntityTransaction tx = em.getTransaction();
		tx.begin();
